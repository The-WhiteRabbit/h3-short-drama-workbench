...
The default is `EXECUTION_ROUTE=newapi-h3-direct`: Agent reads and breaks down the script, prepares character/look prompts and a rolling batch of units after pilot approval, uses image2.5 or user-uploaded images for both looks and storyboards, and binds user-provided voices. Humans review script, assets, whole-unit storyboards, then whole-unit videos in the webpage. “通过本单元并生成” authorizes one H3 job and the local service submits it automatically. Do not ask again in chat. Do not fabricate human approvals.

Use one first-frame reference image for a single-shot video unit; no grid. For a multi-shot video unit, use a 3×3 storyboard made of each shot’s FIRST FRAME in shot order, not multiple moments from one shot. Set `shot_count` in the plan and unit metadata. Leave unused cells blank; never add shots to fill the grid. Review the whole unit together. Preserve uploaded complete sheets as images; individual panels render into nine-cell pages without truncating overflow.

Keep only a compact whole-script plan; prepare the first sample only until its video is approved. Then allow multiple detailed units awaiting review within the current requested batch, while approved units independently enter the H3 queue. There is no single-pending-unit gate; legacy `max_pending_units` is ignored. An unspecified batch is not permission to expand the entire script. New unit IDs include the episode, e.g. `EP001-U001`, and must be globally unique. Register the requested batch with `h3_project.py batch` before post-pilot expansion; an empty batch permits only the initial pilot. Do not rename existing units. Use `deliver` or the webpage delivery button to copy currently approved videos with a provenance manifest, without editing. Do not generate every unit's prompts or images up front. Do not automatically assemble a film. Review edits and uploads persist to local project files. The asset library groups references by purpose and supports review-list membership plus recoverable trash; referenced media cannot be deleted. See the local-project reference for lifecycle and existing-project upgrade rules. Agent continuation is explicit when the Agent is no longer active; the web server is not a background text model.
...
## Required H3 prompt-writing dependency

Before creating or revising any MiniMax H3 video prompt, read the user-designated `h3-prompt-writing` skill and its mode-specific guide. Resolve it from the available skills catalog (installed sibling: `../h3-prompt-writing/SKILL.md`); resolve its references relative to that skill, not this workbench. If it cannot be found, report the missing dependency instead of claiming compliance with an unread guide.

- Identify the actual input mode first. For T2VA, I2VA, FL2VA or L2VA, read `references/base-en.txt` and use `integrated_multimodal_description`, `overall_soundscape`, `non_diegetic_music` in that order.
- For Ref2VA, read `references/ref-en.txt` and use `subject_definitions`, `summary`, `retention_analysis`, `detailed_description`, `overall_soundscape`, `non_diegetic_music` in that order. A reference image is not an exact first/last-frame constraint unless the execution mode supports it.
- Follow that guide's exact field names, section order, reference labels and timing notation. Bind `<Picture N>`, `<Video N>` and `<Audio N>` to the unit's actual ordered inputs; do not invent absent references. Describe observable composition, action, camera and synchronized sound over the requested duration.
- Keep the executable rewrite in English, preserving dialogue, lyrics and visible text in their original language. Also maintain a semantically equivalent Chinese review version; user corrections to actions, left/right hands, watch placement, references or sound must update both versions before review.

For H3 prompt syntax and reference-label semantics, `h3-prompt-writing` takes precedence over older prompt examples in this workbench. This workbench still governs source fidelity, user decisions, bilingual consistency, asset management, review and generation authorization. Reading the dependency does not authorize generation or change the production workflow.
...
Register each received or generated asset before using it downstream. New revisions receive a new asset ID/version and `derived_from`; only one version of a logical asset may be current. Rejected generations remain in the attempt history and are never relabeled as approved.
...
For multi-unit, multi-scene, or full-film H3 work, build only the first unit as the calibration preview. Include its timing math, truth ledger, shot plan, voice evidence, English/Chinese prompts, scene shot ladder, and applicable boundary ledger. Complete Phase 3.5 images for that calibration unit before presenting one combined visual/prompt review for approval. In local-project mode, wait for the calibration video approval, then prepare multiple pending units within the current requested batch; corrections affect the current unit and its actual dependents.

## Phase 3.5 — Generate and review storyboard images (H3)

After the source-grounded unit blueprint and required blocking, create draft shot plans and the matching paired prompts. Generate the calibration unit's actual panel images using available image-generation tools and approved assets; follow [reference/STORYBOARD_PREVIEW.md](reference/STORYBOARD_PREVIEW.md). Keep shot IDs, times and input roles identical across images, the manifest and both prompts. Render the review HTML, inspect actual pixels, fix known errors, then present it for the user's decision. For larger scope, expand only within existing authorization. Do not pad narrative content to fill nine cells.

Visual review and the existing calibration prompt review can be completed together. Record the actual decision for the exact current unit; changing a source, reference, panel or prompt invalidates that unit's approval. The `approve` command records a real user decision, it does not supply one. Source changes may invalidate multiple units; preserve unrelated units when their inputs did not change.
...
For H3, link the current image review page before each unit's upload checklist, with an honest missing/draft/reviewed status. Keep the core production HTML and its existing linter structure. Include one Chinese Director Storyboard copy control and one English six-section copy control per unit. Include the unit blueprint summary, `VOICE_BIBLE`, `POST_NODES`, runtime totals, scene shot ladder, pairwise boundary ledgers, and per-unit planning metadata.
...
- `reference/STORYBOARD_PREVIEW.md` — new panel-image workflow and manifest contract.
- `reference/ASSET_MANAGEMENT.md` — project directories, asset IDs, versions, provenance and generation attempt records.
- `reference/LOCAL_COMFYUI_MINIMAX.md` — existing local execution route, credentials and queue handling.
- `scripts/h3_storyboard.py` — render/review receipts/export/submit/status.
- `scripts/h3_storyboard_tests.py` — offline boundary, mapping and submission tests.
- `templates/storyboard_review.html` — self-contained image review and opinion export.
