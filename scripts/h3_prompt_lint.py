...
TITLE = re.compile(r"^中文审稿版｜H3-[\w-]+｜\d+(?:\.\d+)?秒\s*$", re.MULTILINE)
ZH_SHOT = re.compile(
...
def lint_chinese(prompt: str, index: int) -> tuple[list[str], list[re.Match[str]]]:
    ...
    if TITLE.search(prompt) is None:
        errors.append("missing title in the exact form ‘中文审稿版｜H3-XXX｜N秒’")
...
def main() -> int:
    ...
    if len(blocks) != len(chinese) + len(english):
        errors.append("HTML: one or more prompt-block containers are neither canonical Chinese nor canonical English")
    ...
    if errors:
        ...
        for error in errors:
            print(f"- {error}")
        return 1
    print(f"PASS: {len(chinese)} paired H3 Chinese storyboard + English six-section prompt(s) satisfy structural and parity lint checks")
    return 0
