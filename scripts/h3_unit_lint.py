...
CHINESE_TITLE_RE = re.compile(r"^中文审稿版｜H3-[\w-]+｜\d+(?:\.\d+)?秒\s*$", re.MULTILINE)
...
def main() -> int:
    ...
    for index, unit in enumerate(units, start=1):
        ...
        if len(blocks) != 2 or len(chinese_blocks) != 1 or len(english_blocks) != 1:
            computed_chars = -1
            unit_errors.append(
                "expected exactly one Chinese director-storyboard block and one English "
                f"six-section block, found {len(chinese_blocks)} Chinese / "
                f"{len(english_blocks)} English / {len(blocks)} total"
            )
        else:
